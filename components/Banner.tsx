...
const Banner = ({ netflixOriginals }: props) => {
  ...
  return (
    <div className="flex flex-col space-y-2 py-16 md:space-y-4 md:h-[75vh] justify-center lg:pb-12">
      ...
      <h1 className="ml-2 md:ml-4 text-2xl font-bold md:text-3xl lg:text-5xl">{currentMovie?.name || currentMovie?.title || currentMovie?.original_name}</h1>
      <p className="ml-2 md:ml-4 max-w-xs text-xs  md:max-w-lg md:text-lg lg:max-w-2xl lg:text-2xl leading-5">{currentMovie?.overview}</p>
